...
class UserContainer : DataContainer{
    ...
    static var instance = UserContainer()
    ...
    func getGroup(id: Int) -> GroupData?{
        groupMap[id]
    }
    
    func addGroup(data: GroupData, userId: Int) -> Bool{
        lock()
        defer{unlock()}
        data.isNew = false
        groups.append(data)
    ...
    func updateGroup(data: GroupData, userId: Int) -> Bool{
        var success = false
        lock()
        defer{unlock()}
    ...
    func removeGroup(data: GroupData) -> Bool{
        lock()
        defer{unlock()}
    ...
    func getUser(login: String, pwd: String) -> UserData?{
    ...
    func getUser(login: String) -> UserData?{
        userLoginMap[login]
    }
    
    // user changes
    
    func addUser(data: UserData, userId: Int) -> Bool{
        lock()
        defer{unlock()}
        data.isNew = false
        users.append(data)
    ...
    func updateUser(data: UserData, userId: Int) -> Bool{
        var success = false
        lock()
        defer{unlock()}
    ...
    func updateUserPassword(data: UserData, newPassword: String) -> Bool{
        var success = false
        lock()
        defer{unlock()}
    ...
    func removeUser(data: UserData) -> Bool{
        lock()
        defer{unlock()}
